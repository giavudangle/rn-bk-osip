...
class NewsController {
    async create(req,res) {
        try {
            ...
            const post = await NewsService.create({...req.body, postAt: new Date(), writtenBy: req.payload._id});
    ...
    async update(req, res) {
        try {
            ...
            if(req.payload._id != post.writtenBy ) {
                    return res.status(401).json({
                    success: false,
                    message: "Invalid credentials"
                 })
            ...
            return res.status(200).json({success: true, data: postUpdated});
         } catch (err) {
            console.log(err);
            return res.status(400).json({success: false});
        }
